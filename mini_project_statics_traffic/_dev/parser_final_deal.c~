...
`timescale 1ns/1ps
  ...
  module ethernet_parser
    ...
    (
      ...
      output reg [47:0]                  dst_mac_temp,
      output reg [47:0]                  src_mac_temp,
      output reg                         eth_done,
      output reg [NUM_QUEUES-1:0]        src_port,
		...
		output reg [255:0]						  prj_total_data,
		output reg [255:0]						  prj_mean_data,
		output reg [255:0]						  prj_deviation_data,
 ...
 // ------------ Internal Params --------
   ...
   always @(*) begin
	...
	//	prj_mac_dst5 = 0;
		...
		src_mac_temp = 0;
		dst_mac_temp =0;
		prj_total_data_src1 = 0;
		prj_total_data_src2 = 0;
		prj_total_data_src3 = 0;
		prj_total_data_src4 = 0;
		prj_total_data_src5 = 0;
		prj_total_data_dst1 = 0;
		prj_total_data_dst2 = 0;
		prj_total_data_dst3 = 0;
		prj_total_data_dst4 = 0;
		prj_total_data_dst5 = 0;
 /*-------------------------------------------------------------------------------------------//prj*/
		initialize = 1'b0;		
		end
      state_next       = state;
      case(state)
        ...
        READ_MAC_ADDRESSES: begin
           if(valid) begin
              ...
              dst_mac  = tdata[47:0];
              src_mac  = tdata[95:48];
              src_mac_temp = src_mac;
              dst_mac_temp = dst_mac;
				  eth_done = 1;
              state_next    = WAIT_EOP;
/*-------------------------------------------------------------------------------------------//prj*/
        ...
        prj_tot_cnt_tmp = prj_tot_cnt_tmp + 1;
           if(valid && tlast)begin
              prj_total_data = prj_total_data + (prj_tot_cnt_tmp)  * 256;
              
				  prj_mean_data = 0.9 * prj_mean_data + 0.1 * (prj_tot_cnt_tmp)  * 256;
				  
				  //if (prj_total_data > prj_mean_data)begin
				  prj_deviation_data = 0.9 * prj_deviation_data + 0.1 * ((prj_tot_cnt_tmp)  * 256 - prj_mean_data);
				  //end
				  //else begin
