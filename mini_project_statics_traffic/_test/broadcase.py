...
from reg_defines_reference_switch_nf1_cml import *
...
for i in range(num_broadcast):
    pkt = make_IP_pkt(src_MAC="aa:bb:cc:dd:ee:ff", dst_MAC=routerMAC[0],
                      ...
                      dst_IP="192.168.1.1", pkt_len=100*(i%2+1))
...
if isHW():
    ...
    rres1=nftest_regread_expect(XPAR_NF10_SWITCH_OUTPUT_PORT_LOOKUP_0_LUT_NUM_HITS_REG(), 0) # lut_hit
    print "Checking MISSES register:"
    rres2=nftest_regread_expect(XPAR_NF10_SWITCH_OUTPUT_PORT_LOOKUP_0_LUT_NUM_MISSES_REG(), 0x14)
    mres=[rres1,rres2]
else:
